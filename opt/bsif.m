function [ result ] = bsif( radius,n_sampling,img  )
% filename=('texturefilters/ICAtextureFilters_11x11_8bit');

filename=strcat('texturefilters/ICAtextureFilters_',num2str(2*radius+1),'x',num2str(2*radius+1),'_',num2str(n_sampling),'bit');
...
% bsifcodeim= bsif2(img,ICAtextureFilters,'im');

% unnormalized BSIF code word histogram
result=bsif2(img,ICAtextureFilters,'h');

% % normalized BSIF code word histogram
% bsifhistnorm=bsif2(img, ICAtextureFilters,'nh');
...
function bsifdescription = bsif2(img,texturefilters,mode)
