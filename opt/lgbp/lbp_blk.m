...
wrownum = floor(size(I,1) / floor(rowstep/2) - 1);
wcolnum = floor(size(I,2) / floor(colstep/2) - 1);
lbp_blk = [];

for i = 1:wrownum
    for j = 1:wcolnum
        wcrows = (i-1)*floor(rowstep/2) + 1;
        wcrowe = (i-1)*floor(rowstep/2) + rowstep;
        wccols = (j-1)*floor(colstep/2) + 1;
        wccole = (j-1)*floor(colstep/2) + colstep;
%         A(:,:,:)=I( wcrows:wcrowe, wccols:wccole,1:3 );
        lbphist = lbp2(I( wcrows:wcrowe, wccols:wccole), lbp_r, lbp_p, mapping, mode);
