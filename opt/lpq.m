function [ result ] = lpq( radius,~,img  )
img=rgb2gray(img);
winSize=2*radius+1;
decorr=1;
freqestim=1;
mode='h';
result= lpq2(img,winSize,decorr,freqestim,mode);

end

function LPQdesc = lpq2(img,winSize,decorr,freqestim,mode)
...
% Run first filter
filterResp=conv2(conv2(img,w0.',convmode),w1,convmode);%卷积
...
if decorr == 1
    ...
    [xp,yp]=meshgrid(1:winSize,1:winSize);%直角坐标系
    pp=[xp(:) yp(:)];
    dd=dist(pp,pp');%欧式几何距离
    C=rho.^dd;
    ...
    freqResp=reshape(freqResp,[freqRow*freqCol,freqNum]);%改变输入数组的维数
...
for i=1:freqNum
    LPQdesc=LPQdesc+(double(freqResp(:,:,i))>0)*(2^(i-1));%编码
end
