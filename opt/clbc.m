function [ result ] = clbc( radius,n_sampling,img )
I=img;
mapping=0;
result=clbCount(I,radius,n_sampling,mapping,'h');


end


function result = clbCount(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.3.2
...
if isstruct(mapping)
    bins = mapping.num;
    for i = 1:size(resultd,1)
        for j = 1:size(resultd,2)
            resultd(i,j) = mapping.table(resultd(i,j)+1);
            resultm(i,j) = mapping.table(resultd(i,j)+1);
        end
    end
end
...
if (strcmp(mode,'h') || strcmp(mode,'hist') || strcmp(mode,'nh'))
    ...
    resultd=hist(resultd(:),0:neighbors);
    resultm=hist(resultm(:),0:neighbors);
    resultc=hist(resultc(:),0:1);
    result=[resultd resultm resultc];
