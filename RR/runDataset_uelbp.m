...
featureFcnPath = genpath( 'feature/UELBP' );
% featureType = 'LBP_DBC_SCHARR';
% featureFcn = @(img, opts) lbp_dbc_scharr( opts.dim_mfs, opts.radius, opts.n_sampling, img );
featureType = 'UELBP_INT';
featureFcn = @(img, opts) uelbp_int(opts.a,opts.b, opts.n_sampling, img );

saveDir = [ 'result', filesep, featureType ];
featuresDir = [ saveDir, filesep, 'features' ];

featureOpts2StringFcn = @(opts) [num2str(opts.a), ...
   '_',num2str(opts.b), '_', num2str(opts.n_sampling) ];
