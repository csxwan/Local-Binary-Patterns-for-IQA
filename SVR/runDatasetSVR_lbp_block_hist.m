function runDatasetSVR_lbp_block_hist
%% Setup environment
close all;
clear all;
...
featureFcnPath = genpath( 'feature/LBP_block_hist' );
% featureType = 'LBP_DBC_SCHARR';
% featureFcn = @(img, opts) lbp_dbc_scharr( opts.dim_mfs, opts.radius, opts.n_sampling, img );
featureType = 'LBP_block_hist';
featureFcn = @(img, opts)lbp_block_hist(opts.radius, opts.n_sampling, img );
