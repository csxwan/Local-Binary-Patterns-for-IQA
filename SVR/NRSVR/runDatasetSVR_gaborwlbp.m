function runDatasetSVR_gaborwlbp
%% Setup environment
close all;
clear all;
...
featureFcnPath = genpath( 'feature/GaborLBP' );
% featureType = 'LBP_DBC_SCHARR';
% featureFcn = @(img, opts) lbp_dbc_scharr( opts.dim_mfs, opts.radius, opts.n_sampling, img );
featureType = 'GaborLBP';
featureFcn = @(img, opts)gaborwlbp(opts.radius, opts.n_sampling, img );
