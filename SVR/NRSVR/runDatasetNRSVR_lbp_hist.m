function runDatasetNRSVR_lbp_hist
%% Setup environment
close all;
clear all;
...
featureFcnPath = genpath( 'feature/LBP_HIST' );
% featureType = 'LBP_DBC_SCHARR';
% featureFcn = @(img, opts) lbp_dbc_scharr( opts.dim_mfs, opts.radius, opts.n_sampling, img );
featureType = 'LBP_HIST_INT';
featureFcn = @(img, opts)lbp_hist_int0(opts.radius, opts.n_sampling, img );
...
runDataset_NRSVR(datasetNames, featureInfo);
