function [M,P] = imgaborfilter(varargin)
...
GaborBank=GaborBank.G;
[M,P] = applyGaborFilterFFT(A,GaborBank);
end

function [M,P] = applyGaborFilterFFT(a,GaborBank)
outSize = size(a);
...
for p = 1:length(GaborBank)
    out(:,:,p)=imfilter(a,GaborBank{p},'same');
end
